...
use super::{record::Client, ser::shrink_to_fit, Settings};
use crate::error::{ErrorKind, Result};
use crate::{reset, reset_all, wipe, wipe_all};

pub struct Engine<'a> {
    pub interruptee: &'a dyn Interruptee,
    pub client: &'a Sync15StorageClient,
...
impl<'a> Engine<'a> {
    ...
    ///      apply to it.
    ///   3. It doesn't persist state directly, but relies on Sync Manager
    ///      consumers to persist `ClientSettings`, and on enabled stores to
    ///      wipe and reset themselves.
    ...
    fn apply_incoming(&self, inbound: IncomingChangeset) -> Result<OutgoingChangeset> {
        ...
        for (payload, _) in inbound.changes {
            ...
            if client.id == self.settings.fxa_device_id {
                ...
                for c in client.commands {
                    ...
                    let result = match c.name.as_str() {
                        "wipeEngine" => c.args.get(0).map(|s| s.as_str()).map(wipe),
                        "wipeAll" => Some(wipe_all()),
                        "resetEngine" => c.args.get(0).map(|s| s.as_str()).map(reset),
                        "resetAll" => Some(reset_all()),
                        _ => None,
                    }
                    .unwrap_or_else(|| Err(ErrorKind::UnsupportedCommand(c.name.clone()).into()));
                    if let Err(e) = result {
                        ...
                        log::warn!("Don't know how to apply command {:?}: {:?}", c, e);
